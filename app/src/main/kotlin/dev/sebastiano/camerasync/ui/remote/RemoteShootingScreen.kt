...
private fun RemoteShootingContent(
    ...
    Scaffold(
        ...
        Column(
            ...
            // 5. Advanced Shooting Controls (if supported)
            ...
            // 6. Image Control (Ricoh specific)
            if (
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Row(
            ...
            if (capabilities.batteryMonitoring.supported) {
                val batteryFlow = remember(delegate) { delegate.observeBatteryLevel() }
            ...
            if (capabilities.storageMonitoring.supported) {
                val storageFlow = remember(delegate) { delegate.observeStorageStatus() }
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(16.dp)) {
                if (capabilities.advancedShooting.supportsExposureModeReading) {
                    val exposureModeFlow = remember(delegate) { delegate.observeExposureMode() }
                ...
                if (capabilities.advancedShooting.supportsDriveModeReading) {
                    val driveModeFlow = remember(delegate) { delegate.observeDriveMode() }
                ...
                if (
                    ...
                    val focusFlow = remember(delegate) { delegate.observeFocusStatus() }
                    if (focusFlow != null) {
