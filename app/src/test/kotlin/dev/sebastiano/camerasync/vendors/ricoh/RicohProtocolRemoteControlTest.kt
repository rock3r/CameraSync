...
class RicohProtocolRemoteControlTest {
    ...
    fun `decodeCaptureStatus detects countdown`() {
        val s = RicohProtocol.decodeCaptureStatus(byteArrayOf(1, 0))
    ...
    fun `decodeCaptureStatus detects capturing`() {
        assertEquals(CaptureStatus.Capturing, RicohProtocol.decodeCaptureStatus(byteArrayOf(0, 1)))
    }
    ...
    fun `decodeCaptureStatus capturing value 2 is still Capturing`() {
        assertEquals(CaptureStatus.Capturing, RicohProtocol.decodeCaptureStatus(byteArrayOf(0, 2)))
    }

    @Test
    fun `decodeCaptureStatus countdown value 2 is still Countdown`() {
        val s = RicohProtocol.decodeCaptureStatus(byteArrayOf(2, 0))
        assertTrue(s is CaptureStatus.Countdown)
    }
    ...
    fun `decodeCaptureStatus treats countdown byte as unsigned so 0x80 yields Countdown not Idle`() {
        ...
        // false.
        val s = RicohProtocol.decodeCaptureStatus(byteArrayOf(0x80.toByte(), 0))
        assertTrue(s is CaptureStatus.Countdown)
    }
    ...
    fun `decodeCaptureStatus treats capturing byte as unsigned so 0x80 yields Capturing not Idle`() {
        ...
        assertEquals(
            CaptureStatus.Capturing,
            RicohProtocol.decodeCaptureStatus(byteArrayOf(0, 0x80.toByte())),
        )
    }
    ...
    @Test
    fun `decodeDriveMode values 5 to 9 are MULTI_EXPOSURE`() {
        for (v in 5..9) {
            assertEquals(
    ...
    @Test
    fun `decodeDriveMode value 16 is UNKNOWN`() {
        assertEquals(DriveMode.UNKNOWN, RicohProtocol.decodeDriveMode(byteArrayOf(16)))
    }
    ...
    @Test
    fun `decodeShootingMode mode byte 2 is STILL_IMAGE`() {
